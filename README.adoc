...
. link:JT/05_postman_networking_basics/1_Introduction.adoc[Postman. Networking basics]
. link:JT/02_maven_dependency_management/What_is_Maven.adoc[Maven & Dependency management]
...
. link:JT/06_clean_code_naming_convention/Java_clean_code_and_naming_convention_principles.adoc[Clean code. Naming conventions]
...
====
[start=16]
. link:TL/16_SSH_and_Terminal_commands_and_setup_WSL/0_General_Table_of_Contents.adoc[SSH & Terminal commands. Setup WSL]
...
. link:TL/18_configuring_a_project_and_property_files/Configuring_a_project.Property_files_README.adoc[Configuring a project. Property files]
. link:TL/19_junit_running_junit_tests/JUnit5_overview_and_running_JUnit_tests.adoc[JUnit. Running JUnit tests]
. link:TL/20_test_hooks_Before_and_After/Test_Hooks_Before_And_After_README.adoc[Test hooks - Before & After]
...
. link:TL/23_simple_design_patterns_singleton_page_object/Design_Patterns_README.adoc[Simple Design Patterns. Singleton. Page Object]
. link:TL/24_scenario_context/Scenario_Context_README.adoc[Scenario Context]
...
====
[start=29]
. link:ST/29_layered_architecture/Layered_Architecture_README.adoc[Layered (N-tier) architecture]
. link:ST/30_api_model_rest_basics/Api_Model_and_Rest_Basics_README.adoc[API Model // REST basics]
. link:ST/31_sending_and_validating_API_requests_and_configuration_of_API_client/API_requests_and_responces.adoc[Sending and validating API requests. Configuration of API client]
. link:ST/32_api_requests_in_cucumber_tests/api-tests-with-cumber.adoc[API requests in Cucumber tests]
. link:ST/33_common_data_structures_in_network_communication/Common_Data_Structures_In_Network_Communication_README.adoc[Commonly used data structures in network communication (json, byte array, XML etc.)]
. link:ST/34_programming_with_collections_operations/Programming_With_Collections_Operations.adoc[Programming with Collections. Operations(Creating, sorting, filtering)]
. link:ST/35_streams_and_lambda_expressions/Streams_And_Lambda_Expressions.adoc[Streams & lambda expressions]
. link:ST/36_generics_applied/1_2_generics_applied_README.adoc[Generics applied]
. link:ST/37_regex_and_pattern_in_programming/Regular_Expressions_and_patterns.adoc[Regex & patterns in programming]
. link:ST/38_input_output_programming/Input_Output_Programming.adoc[Input/Output programming]
. link:ST/39_exception_handling_and_custom_exceptions/Exception_Handling_Custom_Exceptions_README.adoc[Exception handling. Custom exceptions]
====